...
async function generateApiDocs() {
  ...
  try {
    ...
    // Fix malformed markdown generated by TypeDoc
    console.log('Fixing TypeDoc markdown issues...');
    try {
      const { fixTypeDocMarkdown } = await import('./fix-typedoc-markdown.js');
      const result = await fixTypeDocMarkdown();
      if (result.success) {
        logInfo(`Fixed ${result.filesFixed} files with malformed markdown`);
      }
    } catch (error) {
      logWarning('Failed to fix TypeDoc markdown: ' + error.message);
    }
...
async function checkBrokenLinks() {
  ...
  async function checkFile(filePath) {
    ...
    // Special handling for TypeDoc-generated links with parentheses in anchors
    const linkRegex = /\[([^\]]+)\]\(([^\s]+?)\)/g;
    let match;
  ...
  // Filter out known TypeDoc issues that are fixed by our scripts
  const { filterOutKnownIssues } = await import('./fix-broken-link-detection.js');
  const actualBrokenLinks = filterOutKnownIssues(brokenLinks);

  if (actualBrokenLinks.length > 0) {
    logError('Found broken links:');
    actualBrokenLinks.forEach(({ file, link, text, line }) => {
  ...
  // Log filtered out links as info
  const filteredCount = brokenLinks.length - actualBrokenLinks.length;
  if (filteredCount > 0) {
    logInfo(`Filtered out ${filteredCount} known TypeDoc issues that are automatically fixed`);
  }

  return {
    success: actualBrokenLinks.length === 0,
    brokenLinks: actualBrokenLinks,
    validLinks,
